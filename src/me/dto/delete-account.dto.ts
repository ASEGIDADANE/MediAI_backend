...
export class DeleteAccountDto {
  @ApiPropertyOptional({ description: 'Current password (required if account has a password)' })
  ...
  @ApiPropertyOptional({
    enum: ['DELETE'],
    description: 'Type DELETE to confirm (required for Google-only accounts without password)',
  })
  @ValidateIf((o: DeleteAccountDto) => o.password === undefined)
  @IsOptional()
  @IsIn(['DELETE'], { message: 'confirm must be DELETE for OAuth-only accounts' })
  confirm?: 'DELETE';
}
